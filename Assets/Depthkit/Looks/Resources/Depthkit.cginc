...
void dkVertexPass(float4 vertIn, inout float2 colorTexCoord, inout float2 depthTexCoord, inout float4 vertOut)
{
    ...
    {
        // flip basetex.y before creating the color and depth texture coordinates to account for the flipped incoming texture
        basetex.y = 1.0 - basetex.y;
        colorTexCoord = basetex * float2(1.0, 0.5);
        depthTexCoord = colorTexCoord + float2(0.0, 0.5);
    }
    else
    {
        // 0,0 is at the bottom left of the image
        depthTexCoord = basetex * float2(1.0, 0.5);
        colorTexCoord = depthTexCoord + float2(0.0, 0.5);
        // flip basetex.y to after creating color and depth coordinates to make 
        // the texturespace coordinate system and world space coordinate system align
        basetex.y = 1.0 - basetex.y;
    }
    ...
    if (depth <= _ClipEpsilon || depth >= (1.0 - _ClipEpsilon))
    {
...
#else
    //
    // unproject vertex from imagespace to world space
    //

    // get imageSpace UV from texture space UV    
    ...
    // get world space coordinate for vertex
    float2 ortho = (imageCoordinates * _ImageDimensions) - _PrincipalPoint;
    float2 proj = ortho * z / _FocalLength;

    // flip the vertex position in Y to be correct in worldspace
    proj.y = -proj.y;
